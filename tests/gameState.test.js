...
describe('GameStateManager', () => {
  ...
  describe('Game State Consistency Validation', () => {
    ...
    test('should validate consistent game state', () => {
      ...
      stateManager.positionHistory.push(stateManager.getFENPosition(gameState.board, gameState.currentTurn, gameState.castlingRights, gameState.enPassantTarget));
  ...
  describe('En Passant Consistency Validation', () => {
    ...
    test('should validate en passant after two-square pawn move', () => {
      ...
      const result = stateManager.validateEnPassantConsistency(gameState);
      expect(result.isValid).toBe(true);
      expect(result.expectedTarget).toEqual({
        row: 5,
        col: 4
      });
    });
...
describe('GameState Additional Coverage for 95%+', () => {
  ...
  describe('Castling Rights Consistency', () => {
    test('should validate castling rights consistency', () => {
      const result = stateManager.validateCastlingRightsConsistency(game.board, game.castlingRights);
      expect(result).toBe(true);
    });
...
describe('Game State Management', () => {
  ...
  describe('FEN Position Generation', () => {
    test('should generate correct FEN for starting position', () => {
      const fen = stateManager.getFENPosition(game.board, game.currentTurn, game.castlingRights, game.enPassantTarget);
  ...
  describe('Castling Rights Consistency', () => {
    test('should validate consistent castling rights', () => {
      const isConsistent = stateManager.validateCastlingRightsConsistency(game.board, game.castlingRights);
...
describe('Castling Rights Validation', () => {
  ...
  test('should validate castling rights consistency', () => {
    const gameState = game.getGameState();
    const result = stateManager.validateCastlingRightsConsistency(gameState.board, gameState.castlingRights);
    expect(result).toBe(true);
  });
...
describe('GameState Additional Coverage', () => {
  ...
  test('should validate en passant consistency after pawn two-square move', () => {
    ...
    expect(result.actualTarget).toEqual({
      row: 5,
      col: 4
    });
  });
  ...
  test('should detect game phase based on material', () => {
    ...
    lowMaterialBoard[0][0] = {
      type: 'king',
      color: 'black'
    };
    lowMaterialBoard[7][0] = {
      type: 'king',
      color: 'white'
    };
    lowMaterialBoard[0][1] = {
      type: 'pawn',
      color: 'black'
    };
    const endgameState = {
