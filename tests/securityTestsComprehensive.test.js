...
describe('Comprehensive Security Tests', () => {
  ...
  describe('Input Validation Security', () => {
    test('should prevent SQL injection attempts in game IDs', () => {
      const sqlInjectionAttempts = ["'; DROP TABLE games; --", "' OR '1'='1", "'; DELETE FROM users; --", "' UNION SELECT * FROM passwords --", "admin'--", "' OR 1=1 --", "'; EXEC xp_cmdshell('dir'); --", "' AND (SELECT COUNT(*) FROM users) > 0 --"];
    ...
    test('should prevent XSS attacks in player names and chat messages', () => {
      const xssAttempts = ["<script>alert('xss')</script>", "<img src=x onerror=alert('xss')>", "javascript:alert('xss')", "<svg onload=alert('xss')>", "<iframe src=javascript:alert('xss')>", "<body onload=alert('xss')>", "<div onclick=alert('xss')>click me</div>", "';alert('xss');//", "\"><script>alert('xss')</script>", "<script>document.cookie='stolen'</script>"];
    ...
    test('should sanitize and validate all coordinate inputs', () => {
      const maliciousCoordinates = [{
        row: "'; DROP TABLE moves; --",
        col: 0
      }, {
        row: "<script>alert('xss')</script>",
        col: 0
      }, {
        row: "javascript:alert('xss')",
        col: 0
      }, {
        row: "eval('malicious code')",
        col: 0
      }, {
        row: "require('fs').readFileSync('/etc/passwd')",
        col: 0
      }, {
        row: "process.exit(1)",
        col: 0
        ...
        row: "constructor.constructor('return process')().exit()",
        col: 0
      }];
      maliciousCoordinates.forEach(coord => {
        const result = game.makeMove(coord, {
          row: 1,
          col: 1
        });
    ...
    test('should prevent code injection through move notation', () => {
      const codeInjectionAttempts = ["e4; require('child_process').exec('rm -rf /')", "e4'; eval('malicious code'); //", "e4\"; process.exit(1); //", "e4`; ${require('fs').readFileSync('/etc/passwd')}; //", "e4${eval('alert(1)')}", "e4#{system('malicious command')}", "e4{{constructor.constructor('return process')().exit()}}", "e4[[__proto__.polluted = true]]"];
    ...
    test('should validate file path inputs to prevent directory traversal', () => {
      const pathTraversalAttempts = ["../../../etc/passwd", "..\\..\\..\\windows\\system32\\config\\sam", "/etc/shadow", "C:\\Windows\\System32\\config\\SAM", "file:///etc/passwd", "\\\\server\\share\\file", "..%2F..%2F..%2Fetc%2Fpasswd", "..%5c..%5c..%5cwindows%5csystem32%5cconfig%5csam"];
  ...
  describe('Prototype Pollution Prevention', () => {
    ...
    test('should prevent prototype pollution through JSON parsing', () => {
      const maliciousJsonStrings = ['{"__proto__": {"polluted": true}}', '{"constructor": {"prototype": {"polluted": true}}}', '{"board": [], "__proto__": {"polluted": true}}', '{"from": {"row": 0, "col": 0, "__proto__": {"polluted": true}}}'];
  ...
  describe('Buffer Overflow and DoS Prevention', () => {
    test('should handle extremely large input strings without crashing', () => {
      const largeStrings = ['A'.repeat(1000000),
      // 1MB string
      'B'.repeat(10000000),
      // 10MB string (if system allows)
      '\x00'.repeat(100000),
      // Null bytes
      '\xFF'.repeat(100000),
      // High bytes
    ...
    test('should prevent memory exhaustion through large arrays', () => {
      const maliciousArrays = [Array(1000000).fill({
        type: 'pawn',
        color: 'white'
      }),
      // Large piece array
    ...
    test('should handle recursive data structures safely', () => {
      ...
      expect(() => {
        ...
        const result = game.makeMove(recursiveObject, {
          row: 1,
          col: 1
        });
        testUtils.validateErrorResponse(result);
    ...
    test('should prevent stack overflow through deep nesting', () => {
      ...
      expect(() => {
        const result = game.makeMove(deeplyNested, {
          row: 1,
          col: 1
        });
        testUtils.validateErrorResponse(result);
  ...
  describe('Authentication and Authorization Security', () => {
    ...
    test('should prevent game state manipulation by unauthorized users', () => {
      ...
      const unauthorizedAttempts = [() => gameManager.makeMove(gameId, 'hacker', {
        from: {
          row: 6,
          col: 4
        },
    ...
    test('should validate session tokens and prevent session hijacking', () => {
      const gameId = 'SEC003';
      gameManager.createGame(gameId);
      const maliciousTokens = ['fake-token-123', 'admin-override-token', '../../../etc/passwd', '<script>alert("xss")</script>', 'null', 'undefined', '', 'Bearer malicious-jwt-token'];
  ...
  describe('Data Integrity and Tampering Prevention', () => {
    test('should detect and prevent board state tampering', () => {
      ...
      // Remove king
      () => {
        game.board.push(Array(8).fill(null));
      },
      // Add extra row
      () => {
        game.board[0] = null;
    ...
    test('should prevent time-based attacks and race conditions', () => {
      ...
      for (let i = 0; i < 100; i++) {
        concurrentMoves.push(gameManager.makeMove(gameId, 'player1', move));
      }
  ...
  describe('Information Disclosure Prevention', () => {
    test('should not leak sensitive information in error messages', () => {
      const sensitiveAttempts = [() => game.makeMove({
        from: {
          row: -1,
          col: 0
        },
      ...
      }), () => gameManager.joinGame('NONEXISTENT', 'player1'), () => game.makeMove(null), () => game.makeMove(undefined)];
    ...
    test('should not expose internal system information', () => {
      const systemInfoAttempts = [() => game.getSystemInfo && game.getSystemInfo(), () => gameManager.getDebugInfo && gameManager.getDebugInfo(), () => gameState.getInternalState && gameState.getInternalState()];
